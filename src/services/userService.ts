...
export default class UserService implements IUserService {
	...
	public async createUser(dto: IUserDTO): Promise<Result<IUserDTO>> {
		try {
			const exists = await this.repoInstance.exists(dto.email);

			if (exists) return Result.fail<IUserDTO>('User with email=' + dto.email + ' already exists');

			const objOrError = await User.create({
				email: UserEmail.create(dto.email).value,
				firstName: UserName.create(dto.firstName).value,
				lastName: UserName.create(dto.lastName).value,
				role: UserRole.create(dto.role).value,
				hidden: false,
			});

			if (!objOrError.isSuccess) return Result.fail<IUserDTO>(objOrError.error);
	...
	public async getAllUsers(): Promise<Result<IUserDTO[]>> {
		try {
			...
			if (list == null) return Result.fail<IUserDTO[]>('There are no users');
	...
	public async updateUser(dto: IUserDTO): Promise<Result<IUserDTO>> {
		try {
			...
			if (dto.firstName) obj.firstName = UserName.create(dto.firstName).value;
			if (dto.lastName) obj.lastName = UserName.create(dto.lastName).value;
	...
	public async deleteUser(email: string): Promise<Result<IUserDTO>> {
		try {
			...
			if (!exists) return Result.fail<IUserDTO>('No user with email=' + email);
